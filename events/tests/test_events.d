...
void testOneTimeListener() {
    ...
    int callCount = 0;
    auto listener = EventListenerOnce((IEvent event) {
        callCount++;
    });
...
void testDispatcher() {
    ...
    dispatcher.on("test.event", (IEvent event) {
        callCount++;
    });
...
void testDispatcherPriority() {
    ...
    dispatcher.on("test", (IEvent event) {
        order ~= "B";
    }, 0);
...
void testDispatcherOnce() {
    ...
    dispatcher.once("test", (IEvent event) {
        callCount++;
    });
...
void testSubscriber() {
    ...
    class TestSubscriber : DEventSubscriber {
        ...
        override void subscribe(DEventDispatcher dispatcher) {
            dispatcher.on("event1", (IEvent event) {
                count1++;
            });
            
            dispatcher.on("event2", (IEvent event) {
                count2++;
            });
        }
    }
