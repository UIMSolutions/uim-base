...
unittest {
    ...
    dispatcher.on("test.event", (DEvent event) {
        callCount++;
    });
    ...
    string order = "";
    dispatcher.on("priority.test", (DEvent event) {
        order ~= "B";
    }, 0);
    ...
    dispatcher.on("stop.test", (DEvent event) {
        propagationOrder ~= "1";
    ...
    int onceCount = 0;
    dispatcher.once("once.test", (DEvent event) {
        onceCount++;
    });
