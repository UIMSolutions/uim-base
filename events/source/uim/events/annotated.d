...
abstract class DAnnotatedEventHandler : UIMObject {
     ...
     */
    void registerWith(DEventDispatcher dispatcher) @trusted {
        registerAnnotatedListeners(this, dispatcher);
    }
}

unittest {
    writeln("Testing annotated event handlers...");
    ...
    class TestHandler : DAnnotatedEventHandler {
        ...
        void normalPriority(IEvent event) {
            if (priorityOrder == 1) priorityOrder = 2;
        }
    }
    
    auto dispatcher = uim.events.dispatcher.EventDispatcher();
    auto handler = new TestHandler();
    handler.registerWith(dispatcher);
    
    // Test regular listener
    dispatcher.dispatch(uim.events.event.Event("user.login"));
    ...
    dispatcher.dispatch(uim.events.event.Event("user.logout"));
    ...
    // Test priority
    dispatcher.dispatch(uim.events.event.Event("priority.test"));
