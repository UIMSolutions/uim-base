module uim.root.datatypes.jsons.count;
...
unittest {
  ...
  assert(json.countIndices([0, 1, 2]) == 1, "Expected 1 element in the provided indices");
  assert(json.countIndices([0, 1]) == 0, "Expected 0 elements in the provided indices");
...
// #region indices
// #region Json[] with indices and delegate
size_t countMap(Json[] jsons, size_t[] indices, bool delegate(size_t) @safe countFunc) {
  return indices.filter!(index => jsons.hasValue(index) && countFunc(index)).array.length;
}
/// 
unittest {
  version (test_uim_root)
...
size_t countPaths(Json[string] map, string[][] paths, bool delegate(string[]) @safe countFunc) {
  return paths.filter!(path => map.hasValue(path) && countFunc(path)).array.length;
}
/// 
unittest {
  version (test_uim_root)
...
size_t countPaths(Json[string] map, string[][] paths) {
  return paths.filter!(path => map.hasValue(path)).array.length;
}
/// 
unittest {
  version (test_uim_root)
...
unittest {
  ...
  assert(count == 2, "Expected 2 arrays in the Json[string] matching the delegate");
}
