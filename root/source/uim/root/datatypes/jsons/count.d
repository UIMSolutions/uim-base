...
unittest {
  ...
  size_t count = map.countArrays([["first"], ["second"], ["third"], ["fourth"]]);
  assert(count == 2, "Expected 2 elements in the provided paths");
}
...
unittest {
  ...
  size_t count = map.countArrays(
    [[1, 2].toJson, [3, 4].toJson, "string".toJson],
    (Json value) => value.isArray);
  assert(count == 2, "Expected 2 elements in the provided values matching the delegate");
}
...
unittest {
  ...
  assert(count == 2, "Expected 2 elements in the Json[string] matching the delegate");
}
