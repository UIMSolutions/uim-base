...
*****************************************************************************************************************/
module uim.phobos.algorithm.mutations;
 ...
 *     // arr is now [3, 4, 1, 2]
 */
T[] shift(T)(ref T[] values, size_t times) {
  T[] results = [];
...
unittest { // T shift(T)(ref T[] values)
  // Test 1: Shift from a non-empty array
  int[] intArray = [1, 2, 3, 4];
  auto shiftedValue = shift(intArray);
  assert(shiftedValue == 1, "Test 1 failed: Expected 1");
  assert(intArray == [2, 3, 4], "Test 1 failed: Expected [2, 3, 4]");

  // Test 2: Shift from a single-element array
  string[] strArray = ["hello"];
  auto shiftedStr = shift(strArray);
  assert(shiftedStr == "hello", "Test 2 failed: Expected 'hello'");
  assert(strArray == null, "Test 2 failed: Expected null");

  // Test 3: Shift from an empty array
  double[] emptyArray;
  auto shiftedEmpty = shift(emptyArray);
  // TODO: correct - assert(shiftedEmpty == double.init, "Test 3 failed: Expected init");
  assert(emptyArray == [], "Test 3 failed: Expected null");

  // Test 4: Shift from an array with mixed types (if applicable)
  // Note: D arrays are homogeneous, so this test is not applicable.

  // Test 5: Shift from an array of structs
  struct Point {
    int x, y;
  }

  Point[] points = [Point(1, 2), Point(3, 4)];
  auto shiftedPoint = shift(points);
  assert(shiftedPoint == Point(1, 2), "Test 5 failed: Expected Point(1, 2)");
  assert(points == [Point(3, 4)], "Test 5 failed: Expected [Point(3, 4)]");

  // Test 6: Shift from an array of booleans
  bool[] boolArray = [true, false, true];
  auto shiftedBool = shift(boolArray);
  assert(shiftedBool == true, "Test 6 failed: Expected true");
  assert(boolArray == [false, true], "Test 6 failed: Expected [false, true]");
}

unittest { // T[] shift(T)(ref T[] values, size_t times) {
  // Test 1: Shift multiple elements from a non-empty array
  int[] intArray = [1, 2, 3, 4, 5];
  auto shiftedValues = shift(intArray, 3);
  assert(shiftedValues == [1, 2, 3], "Test 1 failed: Expected [1, 2, 3]");
  assert(intArray == [4, 5], "Test 1 failed: Expected [4, 5]");

  // Test 2: Shift all elements from the array
  intArray = [10, 20, 30];
  shiftedValues = shift(intArray, 3);
  assert(shiftedValues == [10, 20, 30], "Test 2 failed: Expected [10, 20, 30]");
  assert(intArray == null, "Test 2 failed: Expected null");

  // Test 3: Shift more elements than the array contains
  intArray = [7, 8, 9];
  shiftedValues = shift(intArray, 5);
  assert(shiftedValues == [7, 8, 9], "Test 3 failed: Expected [7, 8, 9]");
  assert(intArray == null, "Test 3 failed: Expected null");

  // Test 4: Shift zero elements
  intArray = [1, 2, 3];
  shiftedValues = shift(intArray, 0);
  assert(shiftedValues == [], "Test 4 failed: Expected []");
  assert(intArray == [1, 2, 3], "Test 4 failed: Expected [1, 2, 3]");

  // Test 5: Shift from an empty array
  int[] emptyArray;
  shiftedValues = shift(emptyArray, 2);
  assert(shiftedValues == [], "Test 5 failed: Expected []");
  assert(emptyArray == [], "Test 5 failed: Expected []");

  // Test 6: Shift from an array of strings
  string[] strArray = ["a", "b", "c", "d"];
  auto shiftedStrValues = shift(strArray, 2);
  assert(shiftedStrValues == ["a", "b"], "Test 6 failed: Expected ['a', 'b']");
  assert(strArray == ["c", "d"], "Test 6 failed: Expected ['c', 'd']");

  // Test 7: Shift from an array of structs
  struct Point {
    int x, y;
  }

  Point[] points = [Point(1, 2), Point(3, 4), Point(5, 6)];
  auto shiftedPoints = shift(points, 2);
  assert(shiftedPoints == [Point(1, 2), Point(3, 4)], "Test 7 failed: Expected [Point(1, 2), Point(3, 4)]");
  assert(points == [Point(5, 6)], "Test 7 failed: Expected [Point(5, 6)]");
}
// #endregion shift
